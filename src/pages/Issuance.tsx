...
export default function Issuance({ user, lang }: { user: User, lang: Language }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [tReq, pReq, cReq, sReq] = await Promise.all([
        supabase.from('transactions').select('*, products(name, unit, cost_price, sku), clients(name, id)').eq('type', 'issuance').in('warehouse_id', role.warehouses).order('created_at', { ascending: false }),
  ...
  const handleUpdateItem = async () => {
    ...
    if (diff > (currentStock?.on_hand || 0)) { alert("Omborda yetarli qoldiq yo'q!"); return; }

    setSaving(true)
    try {
      await supabase.from('transactions').update({ qty: editForm.qty, sell_price: editForm.sell_price, note: editForm.note }).eq('id', editItem.id)
  ...
  const addToCart = () => {
    ...
    setCart([...cart, {
      ...
      attrs: currentStock?.attrs || {},
      variant_label: `${currentStock?.batch ? `[${currentStock.batch}] ` : ''}${Object.values(currentStock?.attrs || {}).join(' × ')}`
    }])
    setActiveProdId(''); setActiveStockId(''); setActiveQty(''); setActivePrice('');
  }

  const removeFromCart = (id: string) => setCart(prev => prev.filter(item => item.tempId !== id))
  ...
  const groupedByClient = useMemo(() => {
    ...
    txs.forEach((tx: any) => {
      ...
      if (!groups[key]) groups[key] = { id: key, client_name: tx.clients?.name || tr.noClient, items: [], total_qty: 0, total_amount: 0 }
  ...
  return (
    <div className="space-y-6 pt-4">
      ...
      <div className="bg-[#0d1018] border border-[#1e2535] rounded-[32px] overflow-hidden mx-4 shadow-2xl">
        <table className="w-full text-left">
          <thead className="bg-[#131720] border-b border-[#1e2535]">
            <tr className="text-[10px] font-mono text-[#4a5568] uppercase tracking-widest font-bold">
            ...
            <div className="bg-[#0d1018] w-full max-w-5xl rounded-[40px] border border-[#1e2535] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
              <div className="p-8 border-b border-[#1e2535] bg-[#131720]">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h2 className="text-3xl font-black text-white mb-4 uppercase tracking-tighter">{viewingGroup.client_name}</h2>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                      <div className="flex flex-col"><span className="text-[14px] text-[#4a5568] uppercase font-mono mb-1">💰 Savdo</span><span className="text-lg font-black text-white">{fmt(totalRev)}</span></div>
                      <div className="flex flex-col"><span className="text-[14px] text-[#4a5568] uppercase font-mono mb-1">📉 Tan Narxi</span><span className="text-lg font-black text-[#8896ae]">{fmt(totalCst)}</span></div>
                      <div className="flex flex-col"><span className="text-[14px] text-[#4a5568] uppercase font-mono mb-1">💹 Foyda</span><span className={`text-lg font-black ${net >= 0 ? 'text-[#00d4aa]' : 'text-[#ff4757]'}`}>{fmt(net)}</span></div>
                      <div className="flex flex-col"><span className="text-[14px] text-[#ff4757] uppercase font-mono mb-1">🎁 Tekin</span><span className="text-lg font-black text-[#ff4757]">-{fmt(totalLoss)}</span></div>
                    </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto p-8">
                 <table className="w-full text-left">
                    <thead className="text-[14px] text-[#4a5568] uppercase border-b border-[#1e2535] font-mono"><tr className="font-bold"><th className="pb-4">Mahsulot / SKU / Partiya</th><th className="pb-4 text-center">Miqdor</th><th className="pb-4 text-right">Summa</th><th className="pb-4 text-center">Tahrir</th></tr></thead>
                    <tbody>
                      {viewingGroup.items.map((item: any, i: number) => (
                        <tr key={i} className="border-b border-[#1e2535]/50 hover:bg-white/5 transition-all">
                          <td className="py-4">
                            <div className="font-bold text-white text-[14px]">{item.products?.name} <span className="text-[#00d4aa] text-[10px] ml-1 font-mono">[{item.products?.sku}]</span></div>
                            <div className="text-[14px] text-[#e27005] font-mono uppercase mt-0.5">
                              {item.batch ? `Partiya: ${item.batch}` : ''}
                              {item.attrs && Object.values(item.attrs).length > 0 ? ` • ${Object.values(item.attrs).join(' × ')}` : ''}
                            </div>
                            <div className="text-[9px] font-mono mt-1 uppercase">{item.sale_type === 'free' ? '🎁 Tekin' : item.sale_type === 'debt' ? '🚩 Qarz' : '✅ Naqd'}</div>
                          </td>
                          ...
                          <td className="py-4 text-center"><button onClick={() => { setEditItem(item); setEditForm({ qty: item.qty, sell_price: item.sell_price, note: item.note || '' }); }} className="text-[#ffa502] hover:bg-[#ffa502]/10 p-2 rounded-lg transition-all">✎</button></td>
                        </tr>
                      ))}
                    </tbody>
                 </table>
      ...
      {/* TAHRIRLASH MODALI */}
      {editItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 backdrop-blur-md p-4" onClick={() => setEditItem(null)}>
          <div className="bg-[#0d1018] w-full max-w-md rounded-[32px] border border-[#1e2535] p-8 shadow-2xl animate-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            <h3 className="text-xl font-black text-white mb-6 uppercase">Tahrirlash: {editItem.products?.name}</h3>
            <div className="bg-[#131720] border border-[#1e2535] p-5 rounded-3xl mb-6 flex justify-between items-center">
              <div className="min-w-0 flex-1">
                <p className="font-black text-white truncate text-lg">{editItem.products?.name} <span className="text-[#00d4aa] text-xs font-mono ml-1">[{editItem.products?.sku || ''}]</span></p>
                <p className="text-[10px] text-[#4a5568] uppercase font-mono tracking-widest">{editItem.batch ? `[${editItem.batch}] ` : ''}{Object.values(editItem.attrs || {}).join(' × ') || '—'}</p>
              </div>
            ...
            <div className="space-y-6">
              <div className="relative"><label className="text-[10px] font-mono text-[#4a5568] uppercase block mb-2 font-bold ml-1 tracking-widest">📦 Miqdor (Soni)</label><div className="relative"><input type="number" className="w-full bg-[#131720] border border-[#1e2535] p-4 rounded-2xl text-white outline-none focus:border-[#ffa502] font-black text-xl" value={editForm.qty} onChange={e => setEditForm({ ...editForm, qty: Number(e.target.value) })}/><span className="absolute right-5 top-1/2 -translate-y-1/2 text-[#4a5568] text-xs font-mono">DONA</span></div></div>
              <div className="relative"><label className="text-[10px] font-mono text-[#4a5568] uppercase block mb-2 font-bold ml-1 tracking-widest">💰 Sotuv Narxi</label><div className="relative"><input type="number" className="w-full bg-[#131720] border border-[#1e2535] p-4 rounded-2xl text-white outline-none focus:border-[#ffa502] font-black text-xl pl-8" value={editForm.sell_price} onChange={e => setEditForm({ ...editForm, sell_price: Number(e.target.value) })}/><span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#ffa502] font-black text-lg">$</span></div></div>
            </div>
            <div className="flex gap-3 mt-8"><button onClick={() => setEditItem(null)} className="flex-1 px-6 py-4 rounded-2xl border border-[#1e2535] text-[#8896ae] font-bold hover:bg-white/5 transition-all text-xs tracking-widest">BEKOR</button><button onClick={handleUpdateItem} disabled={saving} className="flex-1 bg-[#00d4aa] text-[#0d1018] font-black px-6 py-4 rounded-2xl hover:bg-[#00f0c0] transition-all text-xs tracking-widest">SAQLASH</button></div>
          </div>
        </div>
      )}
      
      {/* YANGI CHIQIM MODALI (Cart tizimi) */}
      {modal && (
          ...
          <div className="bg-[#0d1018] w-full max-w-6xl h-[90vh] rounded-[40px] border border-[#1e2535] flex overflow-hidden shadow-2xl">
            <div className="p-8 border-r border-[#1e2535] w-5/12 space-y-5 overflow-y-auto bg-[#0d1018]">
                ...
                <div className="relative">
                  <button onClick={() => setProdDropOpen(!prodDropOpen)} className="w-full bg-[#131720] border border-[#1e2535] p-4 rounded-2xl text-left text-white flex justify-between items-center">{products.find(p => p.id === activeProdId)?.name ? `[${products.find(p => p.id === activeProdId)?.sku}] ${products.find(p => p.id === activeProdId)?.name}` : tr.search} <span>▼</span></button>
                  {prodDropOpen && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-[#131720] border border-[#28324a] rounded-2xl z-50 max-h-72 overflow-y-auto p-2 shadow-2xl">
                      <div className="sticky top-0 bg-[#131720] pb-2 px-1"><input type="text" autoFocus className="w-full bg-[#0d1018] p-3 rounded-xl text-white border border-[#1e2535] outline-none" value={prodSearch} onChange={e => setProdSearch(e.target.value)} placeholder="SKU / Nomi / Textura..."/></div>
                      {filteredProducts.map(p => (
                        <button key={p.id} onClick={() => {setActiveProdId(p.id); setProdDropOpen(false); setProdSearch(''); setActiveStockId('');}} className="w-full p-4 text-left rounded-xl hover:bg-[#ffa502] hover:text-[#0d1018] text-white transition-all">
                          ...
                          <div className="flex gap-2 mt-1 opacity-50 font-mono text-[9px] uppercase tracking-widest"><span className="text-[#00d4aa]">SKU: {p.sku || '—'}</span>{p.attrs?.texture && <span>• TEXTURA: {p.attrs.texture}</span>}</div>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                {activeProdId && (
                  <select className="w-full bg-[#ffa502]/10 border border-[#ffa502]/30 p-4 rounded-2xl text-[#ffa502] outline-none font-bold" value={activeStockId} onChange={e => {setActiveStockId(e.target.value); const s = stockRows.find(sr => sr.id === e.target.value); setActivePrice(s?.sell_price || '');}}>
                    <option value="">--- Partiya va Razmer ---</option>
                    {availableStock.map(s => <option key={s.id} value={s.id}>{s.batch ? `[${s.batch}] ` : ''}{Object.values(s.attrs || {}).join(' × ') || 'Asosiy'}{` | Mavjud: ${s.on_hand}`}</option>)}
                  </select>
                )}
                ...
                <button onClick={addToCart} className="w-full bg-[#00d4aa] text-[#0d1018] font-black py-5 rounded-2xl shadow-lg active:scale-95 transition-all text-lg">+ {tr.add.toUpperCase()}</button>
                <div className="h-px bg-[#1e2535] my-4" />
                <select className="w-full bg-[#131720] border border-[#1e2535] p-4 rounded-2xl text-white outline-none" value={selectedClientId} onChange={e => setSelectedClientId(e.target.value)}><option value="">{tr.client}...</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                <select className="w-full bg-[#131720] border border-[#1e2535] p-4 rounded-2xl text-white outline-none" value={saleType} onChange={e => setSaleType(e.target.value as any)}><option value="paid">✅ NAQD SOTUV</option><option value="debt">🚩 QARZGA BERISH</option><option value="free">🎁 TEKIN (BONUS)</option></select>
            </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-4">
                  {cart.map(item => (
                    <div key={item.tempId} className="bg-[#0d1018] p-5 rounded-[24px] border border-[#1e2535] flex justify-between items-center group animate-in slide-in-from-right duration-300">
                      <div className="min-w-0 flex-1"><p className="font-black text-white truncate text-lg">{item.product_name} <span className="text-[#00d4aa] text-[11px] ml-2 font-mono font-bold">[{item.sku}]</span></p><small className="text-[#4a5568] uppercase font-mono tracking-widest text-[9px]">{item.variant_label}</small></div>
                      <div className="flex items-center gap-6">
                        <div className="text-right font-mono"><p className="text-[10px] text-[#8896ae]">{item.qty} x {fmt(item.sell_price)}</p><p className="font-black text-[#00d4aa] text-lg font-mono">{fmt(item.qty * item.sell_price)}</p></div>
