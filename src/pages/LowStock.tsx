      ...
      <div className="bg-[#0d1018] border border-[#1e2535] rounded-2xl overflow-hidden">
        ...
        {loading ? (
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                {['SKU', 'Razmer', tr.name, 'Ombor', tr.onHand, tr.threshold, 'Yetishmaydi', tr.status].map(h => (
                  <th
                    key={h}
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-16 text-[#4a5568]">
                ...
                return (
                  <tr key={r.id} className="border-b border-[#1e2535] hover:bg-[#131720] transition-all">
                    ...
                    <td
                      ...
                      style={{ color: r.on_hand === 0 ? '#ff4757' : '#ffa502' }}
                    >
                      {r.on_hand}
                    </td>
                    ...
                    <td className="px-4 py-3 font-mono text-[#ff4757] font-bold">
                      −{missing}
                    </td>

                    <td className="px-4 py-3">
                      {r.on_hand === 0 ? (
                        <span className="inline-flex px-2 py-0.5 rounded text-[11px] font-bold font-mono bg-[#ff4757]/10 text-[#ff4757] border border-[#ff4757]/20">
                          {tr.finished}
                        </span>
                      ) : (
                        <span className="inline-flex px-2 py-0.5 rounded text-[11px] font-bold font-mono bg-[#ffa502]/10 text-[#ffa502] border border-[#ffa502]/20">
                          {tr.low}
                        </span>
                      )}
                    </td>
