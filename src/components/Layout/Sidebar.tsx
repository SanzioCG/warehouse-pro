import { useState } from 'react' // Holatni boshqarish uchun qo'shildi
...
export default function Sidebar({ user, page, setPage, lang, lowStockCount, onLogout }: Props) {
  ...
  // Sahifa almashganda menyuni yopish (mobil uchun)
  ...
  return (
    <>
      {/* 1. Mobil uchun Hamburger Tugmasi */}
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="lg:hidden fixed top-3 left-3 z-[60] w-10 h-10 bg-[#0d1018] border border-[#1e2535] rounded-xl flex items-center justify-center text-xl shadow-lg"
      >
        {isOpen ? '✕' : '☰'}
      </button>

      {/* 2. Mobil Backdrop (Fonni qoraytirish) */}
      {isOpen && (
        <div 
      ...
      {/* 3. Sidebar Asosiy Konteyneri */}
      <div className={`
        fixed lg:static inset-y-0 left-0 z-[55]
        w-64 min-w-[256px] bg-[#0d1018] border-r border-[#1e2535] 
      ...
      `}>
        ...
        <div className="p-4 border-b border-[#1e2535] flex items-center gap-3">
          ...
          <div>
            <div className="font-black text-[15px] tracking-tight text-white">{tr.appName}</div>
        ...
        <div className="px-4 py-3 border-b border-[#1e2535] flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="text-[13px] font-semibold truncate text-white">{user.name}</div>
